...
class Server extends Base {
  constructor(server, path, services, wsdl, options) {
    ...
    debug(
      "Server parameters: path: %s services: %j wsdl: %j",
      path,
      services,
      wsdl
    );
    ...
    wsdl.load(function (err) {
      if (err) throw err;
      self.xmlHandler = new XMLHandler(
        self.wsdl.definitions.schemas,
        self.wsdl.options
      );
  ...
  _executeMethod(options, req, callback) {
    ...
    function handleResult(error, result) {
      ...
      var operation =
        self.wsdl.definitions.services[serviceName].ports[portName].binding
      ...
      debugDetail(
        "Server handleResult. operationDescriptor: %j ",
        operationDescriptor
      );

      var outputBodyDescriptor = operationDescriptor.output.body;
      debugDetail(
        "Server handleResult. outputBodyDescriptor: %j ",
  ...
  _sendError(operation, error, callback, includeTimestamp) {
    ...
    debugDetail(
      "Server sendError. operationDescriptor: %j ",
      operationDescriptor
    );
