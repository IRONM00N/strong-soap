...
class Restriction extends XSDElement {
  ...
  postProcess(defintions) {
    ...
    if (this.$base) {
      if (
        this.parent.name === "simpleContent" ||
        ...
        this.base = this.resolveSchemaObject(
          schemas,
          "complexType",
          this.$base
        );
  ...
  enforce(val) {
    ...
    if (this.base) {
      if (this.base.jsType === Boolean) {
        ...
        typeof this.base.jsType === "function" &&
        this.base.jsType !== Date &&
        this.base.jsType !== Number
      ) {
    ...
    if (this.minExclusive !== undefined) {
      if (val <= this.minExclusive) {
        violations.push(
          "value is less or equal than minExclusive (" +
            val +
            " <= " +
            this.minExclusive +
            ")"
    ...
    if (this.maxExclusive !== undefined) {
      if (val >= this.maxExclusive) {
        violations.push(
          "value is greater or equal than maxExclusive (" +
            val +
            " >= " +
            this.maxExclusive +
            ")"
    ...
    if (this.maxInclusive !== undefined) {
      if (val > this.maxInclusive) {
        violations.push(
          "value is greater than maxInclusive (" +
            val +
            " > " +
            this.maxInclusive +
            ")"
    ...
    if (this.totalDigits !== undefined) {
      if (typeof val === "string") {
        ...
        if (totalDigits > this.totalDigits) {
          violations.push(
            "value has more digits than allowed (" +
              totalDigits +
              " > " +
              this.totalDigits +
              ")"
        ...
        if (integerDigits > this.totalDigits) {
          violations.push(
            "value has more integer digits than allowed (" +
              integerDigits +
              " > " +
              this.totalDigits +
              ")"
    ...
    if (this.length !== undefined) {
      if (val.length !== parseInt(this.length)) {
        violations.push(
          "lengths don't match (" + val.length + " != " + this.length + ")"
        );
      }
    }
    ...
    if (this.maxLength !== undefined) {
      if (val.length > this.maxLength) {
        violations.push(
          "length is bigger than maxLength (" +
            val.length +
            " > " +
            this.maxLength +
            ")"
    ...
    if (this.minLength !== undefined) {
      if (val.length < this.minLength) {
        violations.push(
          "length is smaller than minLength (" +
            val.length +
            " < " +
            this.minLength +
            ")"
    ...
    if (violations.length > 0) {
      throw new Error(
        g.f(
          "The field %s cannot have value %s due to the violations: %s",
          this.parent.$name,
          val,
