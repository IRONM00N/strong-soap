...
class Operation extends WSDLElement {
    ...
    switch (child.name) {
      ...
      case "operation": // soap:operation
        ...
        this.soapActionRequired =
          child.$soapActionRequired === "true" ||
          child.$soapActionRequired === "1" ||
          false;
  ...
  describe(definitions) {
    ...
    switch (this.mode) {
      ...
      case Style.rpcEncoded:
        ...
        let nsURI =
          (this.input && this.input.body && this.input.body.namespace) ||
          this.targetNamespace;
        input = new descriptor.ElementDescriptor(
          new QName(nsURI, this.$name),
          null,
          "qualified",
          false
        );
        output = new descriptor.ElementDescriptor(
          new QName(nsURI, this.$name + "Response"),
          null,
          "qualified",
          false
        );
      ...
      case Style.documentEncoded:
        throw new Error(
          g.f("{{WSDL}} style not supported: %s", Style.documentEncoded)
        );
    }
    ...
    this.descriptor.inputEnvelope = Operation.createEnvelopeDescriptor(
      this.descriptor.input,
      false,
      this.soapVersion
    );
    this.descriptor.outputEnvelope = Operation.createEnvelopeDescriptor(
      this.descriptor.output,
      true,
      this.soapVersion
    );
    this.descriptor.faultEnvelope = Operation.createEnvelopeDescriptor(
      this.descriptor.faults,
      true,
      this.soapVersion
    );
  ...
  static createEnvelopeDescriptor(
    parameterDescriptor,
    isOutput,
    soapVersion,
    prefix,
    ...
    if (
      parameterDescriptor &&
      parameterDescriptor.body &&
      !parameterDescriptor.body.Fault
    ) {
      ...
      if (soapVersion === "1.1") {
        faultDescriptor.add(
          new ElementDescriptor(
            new QName(nsURI, "faultcode", prefix),
            null,
            form,
            false
          )
        ...
        faultDescriptor.add(
          new ElementDescriptor(
            new QName(nsURI, "faultstring", prefix),
            null,
            form,
            false
          )
        ...
        faultDescriptor.add(
          new ElementDescriptor(
            new QName(nsURI, "faultactor", prefix),
            null,
            form,
            false
          )
        );
        detailDescriptor = new ElementDescriptor(
          new QName(nsURI, "detail", prefix),
          null,
          form,
          false
        );
        ...
        code.add(
          new ElementDescriptor(
            new QName(nsURI, "Value", prefix),
            null,
            form,
            false
          )
        );
        let subCode = new ElementDescriptor(
          new QName(nsURI, "Subcode", prefix),
          null,
          form,
          false
        );
  ...
  getMode() {
    let use =
      (this.input && this.input.body && this.input.body.use) || "literal";
