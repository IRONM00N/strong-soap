...
describe("wsdl-tests", function () {
  ...
  describe("wsdl parsing test cases", function () {
    it("document/encoded style wsdl is not a supported type", function (done) {
      soap.createClient(
        __dirname + "/wsdl/unsupported/ImportSample.wsdl",
        function (err, client) {
    ...
    it("should not parse connection error", function (done) {
      soap.createClient(
        __dirname + "/wsdl/connection/econnrefused.wsdl",
        function (err, client) {
    ...
    it("should parse external wsdl", function (done) {
      soap.createClient(
        __dirname + "/wsdl/wsdlImport/main.wsdl",
        { strict: true },
        function (err, client) {
          assert.ok(!err);
    ...
    it("should handle element ref", function (done) {
      var expectedMsg =
        ...
        "</ns2:fooRq>\n</soap:Body>\n";
      soap.createClient(
        __dirname + "/wsdl/elementref/foo.wsdl",
        { strict: true },
        function (err, client) {
          assert.ok(!err);
          client.fooOp(
            { fooRq: { paymentRq: { bankSvcRq: { requestUID: "001" } } } },
            function (err, result) {
    ...
    it("should handle type ref", function (done) {
      ...
      soap.createClient(
        __dirname + "/wsdl/typeref/order.wsdl",
        { strict: true },
        function (err, client) {
          assert.ok(!err);
    ...
    it.skip("should load same namespace from included xsd", function (done) {
      var expected = {
        DummyService: {
          DummyPortType: {
            Dummy: {
              name: "Dummy",
              style: "documentLiteral",
              soapAction: "tns#Dummy",
              soapVersion: "1.1",
              input: {
                body: {
                  elements: [
                      ...
                      elements: [
                        {
                          elements: [],
                          attributes: [],
                          qname: { nsURI: "http://www.dummy.com", name: "ID" },
                          type: {
                            nsURI: "http://www.dummy.com/Types",
                            name: "IdType",
                          },
                          form: "qualified",
                          isMany: false,
                          ...
                          elements: [
                            {
                              ...
                              refOriginal: {
                                ...
                                typeDescriptor: {
                                  ...
                                  name: "Dummy",
                                  xmlns: "http://www.dummy.com/Types",
                                  isSimple: false,
                                },
                              },
                            },
      ...
      soap.createClient(
        __dirname + "/wsdl/xsdinclude/xsd_include.wsdl",
        function (err, client) {
          assert.ok(!err);
    ...
    it("handles xsd includes", function (done) {
      soap.createClient(
        ...
        function (err, client) {
          assert.ok(!err);
          var schema =
            client.wsdl.definitions.schemas["http://www.dummy.com/Types"];
    ...
    it("should return type data about anonymous simple types within elements", function (done) {
      openWSDL(
        path.resolve(__dirname, "wsdl/restriction_anonymous_types.wsdl"),
        function (err, def) {
          var operation =
            def.definitions.bindings.RestrictionsBinding.operations
              .TestRestrictions;
          ...
          assert.equal(
            elementWithAnonymousType.qname.name,
            "elementWithAnonymousType"
          );
          // anonymous types have same name as parent element
          assert.equal(
            elementWithAnonymousType.type.name,
            elementWithAnonymousType.qname.name
          );
          // Check if anonymous type restriction is embedded in the parent element
          assert.equal(
            elementWithAnonymousType.type.anonymous.name,
            "simpleType"
          );
          assert.equal(
            elementWithAnonymousType.type.anonymous.type.$name,
            "string"
          );
    ...
    it("should map isMany values correctly", function (done) {
      openWSDL(
        path.resolve(__dirname, "wsdl/marketo.wsdl"),
        function (err, def) {
          var operation =
            def.definitions.bindings.MktowsApiSoapBinding.operations
              .getLeadChanges;
          var operationDesc = operation.describe(def);
          assert(operationDesc.input.body.elements[0].elements);

          // Check that an element with maxOccurs="1" maps to isMany = false
          operationDesc.input.body.elements[0].elements.forEach(function (
            element
          ) {
