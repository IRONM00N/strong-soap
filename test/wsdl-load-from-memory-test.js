  ...
  describe("should load the wsdl from memory", function () {
    ...
    beforeEach(function (done) {
      ...
      promiseList.push(
        new Promise(function (resolve, reject) {
          fs.readFile(
            __dirname + "/wsdl/from-memory/stockquote.wsdl",
            "utf8",
      ...
      promiseList.push(
        new Promise(function (resolve, reject) {
          fs.readFile(
            __dirname + "/wsdl/from-memory/main.wsdl",
            "utf8",
            function (err, definition) {
              if (err) {
                reject(err);
    ...
    it("should fail to load a wsdl synchronously as wsdl not present in memory", function (done) {
      ...
      try {
        ...
        assert.ok(
          err.indexOf(
            "For loadSync() calls all schemas must be preloaded into the cache"
          ) > -1
        );
        done();
      }
      ...
      fs.readFile(
        ...
        function (err, definition) {
          ...
          wsdlDefinition.load(function (err, loadedWsdl) {
            ...
            assert.notDeepEqual(
              loadedWsdl.definitions.bindings,
              {},
              "Bindings not loaded on wsdl"
            );
            assert.notDeepEqual(
              loadedWsdl.definitions.services,
              {},
              "Services not loaded on wsdl"
            );
            assert.notDeepEqual(
              loadedWsdl.definitions.portTypes,
              {},
              "PortTypes not loaded on wsdl"
            );
            done();
          });
        }
