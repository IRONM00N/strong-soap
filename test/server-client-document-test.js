...
describe("Document style tests", function () {
  describe("Document/Literal with simple types param", function () {
    ...
    before(function (done) {
      fs.readFile(
        __dirname + "/wsdl/strict/doc_literal_test.wsdl",
        "utf8",
        function (err, data) {
          assert.ok(!err);
          test.wsdl = data;
    ...
    it("Document/literal test with simpleType params", function (done) {
      soap.createClient(
        test.baseUrl + "/doc_literal_test?wsdl",
        function (err, client) {
          assert.ok(!err);
  ...
  describe("Document/Literal wrapped with simple type params with attribute($attribute)", function () {
    ...
    before(function (done) {
      fs.readFile(
        __dirname + "/wsdl/strict/doc_literal_wrapped_test.wsdl",
        "utf8",
        function (err, data) {
          assert.ok(!err);
          test.wsdl = data;
    ...
    it("Document/literal wrapped test with simpleType params with attribute($attribute)", function (done) {
      soap.createClient(
        test.baseUrl + "/doc_literal_wrapped_test?wsdl",
        function (err, client) {
          assert.ok(!err);
          //
          client.myMethod(requestArgs, function (err, result, body) {
          ...
          myMethod: function (args, cb, soapHeader) {
            throw {
              Fault: {
                ...
                detail: {
                  myMethodFault1: {
                    errorMessage1: "MyMethod Business Exception message",
                    value1: 10,
                  },
                },
              },
  ...
  describe("Document/Literal wrapped with SOAP 1.2 Fault", function () {
    ...
    before(function (done) {
      fs.readFile(
        __dirname + "/wsdl/strict/doc_literal_wrapped_test_soap12.wsdl",
        "utf8",
        function (err, data) {
          assert.ok(!err);
          test.wsdl = data;
    ...
    beforeEach(function (done) {
      ...
      test.server.listen(15099, null, null, function () {
        test.soapServer = soap.listen(
          test.server,
          "/doc_literal_wrapped_test_soap12",
          test.service,
          test.wsdl
        );
        test.baseUrl =
    ...
    it("Document/literal wrapped test with Fault2", function (done) {
      soap.createClient(
        test.baseUrl + "/doc_literal_wrapped_test_soap12?wsdl",
        function (err, client) {
          assert.ok(!err);
          client.myMethod({ x: 200, y: 10.55 }, function (err, result, body) {
            assert.ok(err);
            ...
            assert.ok(index > -1);
            var index = body.indexOf(
              'ns1:myMethodFault2 xmlns:ns1="http://example.com/doc_literal_wrapped_test_soap12.xsd"'
            );
            assert.ok(index > -1);
            var index = body.indexOf(
              "<errorMessage2>MyMethod Business Exception message</errorMessage2>"
  ...
  describe("client options tests", function () {
    ...
    it("Generate request envelope without newlines and white spaces", function (done) {
      var expectedRequest =
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope"><soap:Header/><soap:Body><ns1:myMethod xmlns:ns1="http://example.com/doc_literal_wrapped_test_soap12.xsd"><x>200</x><y>10.55</y></ns1:myMethod></soap:Body></soap:Envelope>';
